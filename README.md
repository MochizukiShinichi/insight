# Insight Data Engineering Challenge -- H1-B statistics

## Problem

The purpose of this project is to create a mechanism to analyze H1-B visa application data, specificially calculate two metrics: **Top 10 Occupations** and **Top 10 States** for **certified** visa applications.

The code should be modular and reusable for any .csv input given it satisfies the requirement in the instruction sets

## Input Dataset

Input file requirements:
1. in .csv or .txt format of one specific visa type (for here sepcifically, H1-B)
2. Each line holds one record and each field on each line is separated by a semicolon (;).
3. put in ./input/ folder
4. having required **columns** and **names**:
    ...
    * **WORKSITE_STATE**: state where the employee will be working at, only 1 primary worksite should be provided
    * **SOC_NAME**: name of the occupation associated with SOC code
    note: variation of lower/upper case of column names are acceptable
...
This program must create 2 output files:
...
Percentages are rounded off to 1 decimal place.

## Example
Given the input file, `./input/h1b_input.csv` with the following data:
```
...
then your will find in ./output/ folder:
